...
//        }
    ...
    private void sendFailedStringCallback(final Request request, final Exception e, final ResultCallback callback) {
        if (callback != null && callback.requestCallBack != null) {
            if (context!=null){
                MJGLUtils.exeGLQueueEvent(context, new Runnable() {
                    @Override
                    public void run() {
                        callback.requestCallBack.onFailure(e, e.getMessage());
                    }
                });
...
//            callback.onError(request, e);
    ...
    //成功处理
    private void sendSuccessResultCallback(final String object, final ResultCallback callback,String url) {
        if (callback != null && callback.requestCallBack != null) {
            if (context!=null){
                MJGLUtils.exeGLQueueEvent(context, new Runnable() {
                    @Override
                    public void run() {
                        callback.requestCallBack.onSuccess(callback.parseResponse(object));
                    }
                });
            } else {
                callback.requestCallBack.onSuccess(callback.parseResponse(object));
    ...
    private BaseViewActivity context;

    /**
     * 设置成 gl线程回调
     * @param context  BaseViewActivity gl上下文
     */
    public  static  void  enableGlthread (BaseViewActivity context){
