...
//	}
	...
	//设置页码默认颜色
	public void setNumDefaultColor( int id ){
		mDefaultColor = id;
	}
...
//	}
	...
	//设置页码选中状态颜色
	public void setNumSelectedColor( int id ){
		mSelectedColor = id;
	}
...
//	}
	...
	public void showPage(){
		...
		if(mIsOpen) {
			HeadControl.bindView(nextBtnImgView);
			nextBtnImgView.setOnKeyListener(keyListener);
		}
		prvBtnImgView = new GLImageView( this.getContext() );
		if(mIsOpen) {
			HeadControl.bindView(prvBtnImgView);
			prvBtnImgView.setOnKeyListener(keyListener);
		}
		if ( ! mbNumListVisible ){
			return;
		}
...
//							view.setBackground(mDefaultColor);
	...
	private GLOnKeyListener keyListener = new GLOnKeyListener(){
		@Override
...
//							requestLayout();
	...
	public void setHeadControl(boolean isOpen) {
		mIsOpen = isOpen;
	}
